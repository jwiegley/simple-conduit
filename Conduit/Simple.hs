...
conduitVectorIO :: (V.Vector v a) => Int -> Conduit a IO (v a)
conduitVectorIO maxSize src = source $ \z yield -> do
    mv <- liftBase $ VM.new maxSize
    EitherT $ do
        eres <- runEitherT $ runSource src (z, 0) $ \(r, i :: Int) x -> EitherT $
            if i >= maxSize
...
{-
-- vectorBuilder :: (PrimMonad base, MonadBase base m,
--                   V.Vector v e, MonadBase base n)
--               => Int -- ^ size
--               -> Conduit i m (v e)
vectorBuilder size src = do
    ref <- liftBase $ do
        mv <- VM.new size
        newMutVar $! S 0 mv id
    sink $ \z yield -> do
        res <- runSource src z $ \r x ->
            addE ref x
            yieldS (liftBase . addE ref)
        vs <- do
...
yieldS :: (PrimMonad base, MonadBase base m)
       ...
       -> m (v e)
yieldS ref z yield = do
    S idx mv front <- liftBase $ readMutVar ref
    r <- foldM yield z (front [])
